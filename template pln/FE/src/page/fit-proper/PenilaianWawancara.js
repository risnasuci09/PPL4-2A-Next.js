...
export default function PenilaianWawancara() {
    ...
    const filterBulan = (value) => {
        ...
        axios
            .get(
                `http://localhost:1337/api/wawancaras?populate=peserta.pegawai.jabatan&filters[jadwal][$gte]=${value}&filters[jadwal][$lte]=${newDate(value)}&populate=nilai_wawancara`
            )
            .then((res) => {
    ...
    return (
        <div className="row mb-2">
            <div className="col-sm-6">
                <h1 className="m-0" style={{ fontWeight: 'normal' }}>PenilaianWawancara</h1>
            </div>
            <div className="col-sm-6">
            ...
            <div className='col-12'>
                <div className="card card-default color-palette-box">
                    ...
                    <div className="card-body">
                        <div className='col-4'>
                            <input className="form-control" type="month" onChange={(e) => filterBulan(e.target.value)} />
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <tbody>
                                    {dataPeserta.length !== 0 ?
                                        dataPeserta.map((res, index) => {
                                            console.log("res penguji", res.attributes.pengujis.data);
                                            return (
                                                <tr key={index}>
                                                    <td>{index + 1}</td>
                                                    ...
                                                    <td>{res.attributes.proyeksi_jabatan_wawancara}</td>
                                                    <td>{formatDate(res.attributes.jadwal_wawancara)}</td>
                                                    <td>
                                                        <DropdownButton title={<i class="fas fa-user-alt"></i>} className='drop'>
                                                            {res.attributes.pengujis.data.map((temp, i) => {
                                                                return (
